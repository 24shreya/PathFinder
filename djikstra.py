...
def algorithm(draw, grid, start, end):
	...
	open_set_hash = {spot: False for row in grid for spot in row}
	...
	while not open_set.empty():
		...
		for neighbour in current.neighbors:
			...
			if new_gscore < g_score[neighbour]:
				...
				if not open_set_hash[neighbour]:
					count+=1
					open_set.put((new_gscore,count,neighbour))
					neighbour.make_open()
